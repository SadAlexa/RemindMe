...
interface TagsOnTaskDAOs {
    ...
    @Query("SELECT * FROM tags LEFT JOIN tasks_tags ON tags.id = tasks_tags.tag_id WHERE tasks_tags.task_title = :taskTitle AND tasks_tags.task_list_title = :listTitle AND tasks_tags.task_user_id = :userId")
    fun getAllTagsOnTask(taskTitle: String, listTitle: String, userId: Long): List<TagEntity>
